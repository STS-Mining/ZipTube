    ...
    with ThreadPool(cpu_count())as p:
        ...
        for Folder in FOLDERS:
            try:
                ...
                pass
            PROCESSES += Folder[1]
        print(f"Converting {len(PROCESSES)} MP3 files to FLAC")
        JOBS = p.map(converttoflac, PROCESSES)
        FAILED_JOBS = []
        for job in JOBS:
            if job.returncode != 0:
                FAILED_JOBS.append(job)
        MESSAGE = (f"Converting Finished! \r {len(PROCESSES)-len(FAILED_JOBS)}/{len(PROCESSES)} "
                   f"MP3 files converted to FLAC in \r{time.time() - STARTTIME:.4f} seconds")
        try:
